...
This document helps diagnose common Network Enforcer failures.
...
. Secret `net-enf-goldmane-client-certs` exists in the Network Enforcer
...
* Empty body: nothing has been ingested since the last drain (or since enable),
  or the buffer already dropped unread records.
...
. Pre-existing Kubernetes `NetworkPolicy` objects in the namespace AND with
  Network Enforcer's policy. A broad default-deny plus a narrow learned allow
